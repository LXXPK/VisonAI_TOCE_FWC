...
# API KEYS
GEMINNI_API_KEY=os.getenv("GEMINI_API_KEY")
...
@app.route('/set_quiz_mode', methods=['POST'])
def set_quiz_mode():
    global quiz_mode
    data = request.get_json()
    
    # Retrieve 'quiz_mode' with a default value of False if not provided
    quiz_mode = data.get('quiz_mode', False)
    
    # Print the value to confirm it is being set correctly
    print(f"Quiz Mode set to: {quiz_mode}")
    
    return jsonify({'quiz_mode': quiz_mode}), 200
...
def process_pdf():
    global pdf_text  # Access and modify the global variable
    try:
        ...
        if pdf_file and pdf_file.filename.endswith('.pdf'):
            ...
            pdf_text = text
            print(pdf_text)  # Optional: For debugging
...
def send_message():
    ...
    genai.configure(api_key=GEMINNI_API_KEY)
    ...
    if(quiz_mode):
            if pdf_text:
                ...
                Your name is easify. You are a personal AI teaching assistant. Your responses will be based entirely on the following text from an uploaded document:

                {pdf_text}

                If the provided text does not contain the answer to the user's question, respond with "I'm sorry, I cannot answer that question based on the provided document."

                User Input: {user_input} and have some interactive multiple choice questions to make the user more attentive and engaging after every small topic.
                Please ensure responses do not contain any emojis, expression symbols, or other extra symbols such as '***' or '##', as these may detract from the user experience when converted to speech.  I need only text; no emojis or special symbols.
                """
            else:
                system_instruction = """
                Your name is easify.  You are a personal AI teaching assistant.  Generate a quiz with multiple-choice questions based on the user's input.   Avoid lengthy responses.  Please ensure responses do not contain any emojis or special symbols.

                User Input: {user_input}
                """
    else:  #This is the non-quiz mode
        if pdf_text:
            system_instruction = f"""
            Your name is easify. You are a personal AI teaching assistant. Your responses will be based entirely on the following text from an uploaded document:
...
def conceptchat():
    if request.method == 'POST':
        ...
        if(pdf_text):
            system_instruction = f"""
            You are a video script generator. You will receive a user request and text. You MUST generate ONLY the script in response; no other text. If the text is insufficient, return "Insufficient data".

            Example:
            User Request: Explain photosynthesis.
            Text: [some text about photosynthesis]
            Script: [a concise script about photosynthesis]
...
# ----------------------------------------------------video making --------------------------------------
            ...
            API_URL = "https://api-inference.huggingface.co/models/goofyai/3D_Render_for_Flux"
